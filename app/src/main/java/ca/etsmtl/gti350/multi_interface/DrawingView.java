...
class CursorContainer {
    ...
    public ArrayList<Point2D> getCursorsPosition() {
        ...
        for (int i = 1; i < cursors.size(); i++) {
            arrayList.add(cursors.get(i).getCurrentPosition());
        }
        return arrayList;
    }
    ...
    public void removeAllCursor() { cursors.clear(); }
}
...
public class DrawingView extends View {
    ...
    ArrayList<Point2D> selectedLassoResultPoints = new ArrayList<Point2D>();
    ...
    static final int MODE_SELECTION_MANIPULATION = 4;
    static final int MODE_EFFACER = 5;
    static final int MODE_ENCADRER = 6;
    static final int MODE_CREER = 7;
    ...
    MyButton lassoButton = new MyButton("Lasso", 10, 170, 140, 140);
    MyButton effacerButton = new MyButton("Effacer", 10, 320, 140, 140);
    MyButton encadrerButton = new MyButton("Encadrer", 10, 470, 140, 140);
    MyButton creerButton = new MyButton("Créer", 10, 620, 140, 140);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        if (selectedShapes.size() > 0) {
            ...
            selectedLassoResultPoints.clear();
            AlignedRectangle2D rect = new AlignedRectangle2D();
            for (Shape s : selectedShapes) {
                for (Point2D p : s.getPoints()) {
                    selectedLassoResultPoints.add(p);
                    rect.bound(p);
                }
            }
            selectedLassoResultPoints = Point2DUtil.computeConvexHull(selectedLassoResultPoints);
            selectedLassoResultPoints = Point2DUtil.computeExpandedPolygon(selectedLassoResultPoints, rect.getDiagonal().length() / 30);

            gw.setColor(1.0f, 0.0f, 0.0f, 0.8f);
            gw.fillPolygon(selectedLassoResultPoints);
        }
        ...
        if (currentMode == MODE_CREER) {
            ArrayList<Point2D> arrayList = cursorContainer.getCursorsPosition();
            if (arrayList != null) {
                gw.setColor(1.0f, 0.0f, 0.0f, 0.5f);
                gw.fillPolygon(arrayList);
            }
            arrayList.clear();
        }
        ...
        if (cursorContainer.getNumCursors() > 0) {
            ...
            gw.drawString(150, 250, "[" + cursorContainer.getNumCursors() + " contacts]");
            gw.drawString(400, 250, "[ currentMode: " + currentMode + " ]");
        }
                ...
                switch (currentMode) {
                    case MODE_NEUTRAL:
                        if (cursorContainer.getNumCursors() == 1 && type == MotionEvent.ACTION_DOWN) {
                            ...
                            } else if (Point2DUtil.isPointInsidePolygon(selectedLassoResultPoints, p_pixels)) {
                    ...
                    case MODE_CREER:
                        if (type == MotionEvent.ACTION_DOWN) {
                            ...
                            cursorContainer.removeAllCursor();
                            if (cursorContainer.getNumCursors() == 0) {
                                currentMode = MODE_NEUTRAL;
                            }
                        }
                    ...
                    case MODE_EFFACER:
                        if (type == MotionEvent.ACTION_DOWN) {
                            Point2D p_world_cursor = gw.convertPixelsToWorldSpaceUnits(cursor.getCurrentPosition());
                            int indexToDelete = shapeContainer.indexOfShapeContainingGivenPoint(p_world_cursor);
